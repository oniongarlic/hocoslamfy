/*
 * Hocoslamfy, blackberry platform-specific code file
 * Copyright (C) 2014 Nebuleon Fumika <nebuleon@gcw-zero.com>
 * Copyright (C) 2014 Kaj-Michael Lang <milang@tal.org>
 *
...
{
	return event->type == SDL_KEYDOWN && (event->key.keysym.sym == SDLK_RETURN || event->key.keysym.sym == SDLK_SPACE);
}

bool IsEnterGameReleasingEvent(const SDL_Event* event)
{
	return event->type == SDL_KEYUP && (event->key.keysym.sym == SDLK_RETURN || event->key.keysym.sym == SDLK_SPACE);
}
...
{
	return event->type == SDL_KEYDOWN && event->key.keysym.sym == SDLK_p;
}
